# This file is originally from project <python-doipclient>https://github.com/jacobschaer/python-doipclient/
import struct
...
class RoutingActivationRequest(DoIPMessage):
    ...
    def pack(self):
        if self._vm_specific is not None:
            ...
            rtn = struct.pack("!BBHI", self._version, self._inverse_version, self.payload_type, len(payload)) + payload

            return rtn
            
    def __init__(self, source_address, activation_type=ActivationType.Default, reserved=0, vm_specific=None):
        self._source_address = source_address
...
class RoutingActivationResponse(DoIPMessage):
    ...
    def pack(self):
        if self._vm_specific is not None:
            ...
            payload = struct.pack(
                "!HHBL", self._client_logical_address, self._logical_address, self._response_code, self._reserved
            )
...
class DiagnosticMessage(DoIPMessage):
    ...
    def pack(self):
        payload =  (
            struct.pack("!HH", self._source_address, self._target_address) + self._user_data
        )
